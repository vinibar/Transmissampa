...
public class RtspClient {
    ...
    private void sendAuthentication() throws NoSuchAlgorithmException, IOException {
        MessageDigest digest = MessageDigest.getInstance("MD5");
        digest.update("hackme".getBytes("UTF-8"));
        ...
        mOutputStream.write(hexString.toString().getBytes("UTF-8"));

        String line = null;

        char[] result = new char[32];
        mBufferedReader.read(result);
        if (result != null) {
            Log.d("Splitter", "Connection accepted");

            //getSession().getVideoTrack().
